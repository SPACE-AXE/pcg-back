...
export class Migration1715511438160 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`parking_transaction\` DROP COLUMN \`car_num\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`parking_transaction\` ADD \`car_num\` varchar(15) NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`parking_transaction\` ADD \`car_num\` int NULL`,
    );
    await queryRunner.query(
