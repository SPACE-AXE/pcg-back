...
export class PaymentService {
  ...
  async pay(user: UserV2, payDto: PayDto) {
  ...
  async addCreditCard(user: UserV2, addCreditCardDto: AddCreditCardDto) {
  ...
  async getCreditCard(user: UserV2) {
    const card = await this.cardRepository.findOne({
      where: { user: { id: user.id } },
      select: {
        id: true,
